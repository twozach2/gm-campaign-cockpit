...
function relayPairBody(value) {
  const body = exactKeys(value, ["pairingToken", "deviceName"]);
  return {
    pairingToken: requiredString(
      body,
      "pairingToken",
      512,
      "Pairing code",
    ),
    deviceName: requiredString(body, "deviceName", 80, "Device name"),
  };
}

function relayConnectBody(value) {
  const body = exactKeys(value, ["roomId"]);
  return {
    roomId: requiredString(body, "roomId", 128, "Room ID"),
  };
}
...
function limit(bucket, scope, capacity, windowMs) {
  return Object.freeze({ bucket, scope, capacity, windowMs });
}

const DM_MUTATION_LIMIT = limit("dm-write", "identity", 120, 60_000);
const PLAYER_MUTATION_LIMIT = limit("player-write", "identity", 60, 60_000);
...
export const API_ROUTES = Object.freeze({
  ...
  "/api/readiness": route("GET", "public"),
  "/api/dm/session": route("GET", "public", {
    rateLimits: [limit("dm-session", "ip", 60, 60_000)],
  }),
  "/api/dm/login": route("POST", "public", {
    parseBody: loginBody,
    rateLimits: [limit("dm-login", "ip", 6, 60_000)],
  }),
  "/api/dm/logout": route("POST", "dm", { rateLimits: [] }),
  "/api/dm/stream-ticket": route("POST", "dm", {
    rateLimits: [
      limit("dm-ticket", "identity", 30, 60_000),
      limit("ticket-ip", "ip", 200, 60_000),
    ],
  }),
  "/api/player/join": route("POST", "public", {
    parseBody: displayNameBody,
    rateLimits: [limit("player-join", "ip", 12, 60_000)],
  }),
  "/api/player/rename": route("POST", "player", { parseBody: displayNameBody }),
  "/api/player/leave": route("POST", "player", { rateLimits: [] }),
  "/api/player/stream-ticket": route("POST", "player", {
    rateLimits: [
      limit("player-ticket", "identity", 30, 60_000),
      limit("ticket-ip", "ip", 200, 60_000),
    ],
  }),
  "/api/stream": route("GET", "ticket", {
    parseQuery: streamQuery,
    rateLimits: [limit("stream-open", "ip", 120, 60_000)],
  }),
  "/api/player/state": route("GET", "player"),
  "/api/dm/state": route("GET", "dm"),
  "/api/relay/state": route("GET", "dm"),
  "/api/relay/pair": route("POST", "dm", {
    parseBody: relayPairBody,
  }),
  "/api/relay/refresh": route("POST", "dm"),
  "/api/relay/connect": route("POST", "dm", {
    parseBody: relayConnectBody,
  }),
  "/api/relay/unpair": route("POST", "dm"),
  "/api/player/image": route("GET", "revealed", {
    parseQuery: imageQuery,
    rateLimits: [limit("player-file", "ip", 240, 60_000)],
  }),
  "/api/chat": route("POST", "dm-or-player", {
    maxBodyBytes: 8_192,
    parseBody: chatBody,
    rateLimits: [
      limit("chat", "identity", 30, 10_000),
      limit("chat-ip", "ip", 150, 10_000),
    ],
  }),
  ...
  "/api/file": route("GET", "dm", {
    parseQuery: fileQuery,
    rateLimits: [
      limit("dm-file", "identity", 240, 60_000),
      limit("dm-file-ip", "ip", 500, 60_000),
    ],
  }),
