...
function players(value, label = "Players") {
  if (!Array.isArray(value) || value.length > 200) {
    throw protocolError(`${label} must contain at most 200 players`);
  }
...
function chatMessage(value, label = "Chat message") {
  ...
  if (scope === "whisper") {
    ...
    const toPlayerId = optionalString(
      record.toPlayerId,
      `${label}.toPlayerId`,
      { max: 128 },
    );
    if (toPlayerId !== undefined) result.toPlayerId = toPlayerId;
    if (fromPlayerId === undefined && toPlayerId === undefined) {
      throw protocolError(`${label} whisper requires a player participant`);
    }
  ...
  if (type === "relay.hello") {
    ...
    return {
      ...
      acceptedRevision: integer(
        ...
        "Relay payload.acceptedRevision",
      ),
    };
  }
  if (type === "heartbeat") {
